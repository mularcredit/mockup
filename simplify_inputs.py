...
def replacer(match):
    ...
    # check for icon
    icon_match = re.search(r'<([A-Z][a-zA-Z]+)\s+className="([^"]+)"\s*/>', inner_html)
    has_icon = False
    icon_html = ""
    if icon_match and "shrink-0" in icon_match.group(2):
        has_icon = True
...
pattern = r'<div className="(col-span-[^"]+ relative)">\s*<div className="relative group mt-2 flex items-center px-3">(.*?)</div>\s*</div>'
content = re.sub(pattern, replacer, content, flags=re.DOTALL)

with open(filepath, 'w') as f:
    f.write(content)

print("Inputs simplified.")
